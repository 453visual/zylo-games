...
--  STATUS: 100% PRESERVED & AUTO-UNEQUIP ON STOP ENABLED
-- =========================================================================
...
return function(PagePets, State, ZyloLib, Main)
    ...
    -- =============================================================
    -- UI ACCORDION AUTO HATCH & PET TEAM MANAGER
    -- =============================================================
    ...
    -- [PERBAIKAN PRESISI]: TOMBOL STOP OTOMATIS MENGANGKAT & MEMASUKKAN SEMUA PET KE INVENTORY
    ...
    StopBtn.MouseButton1Click:Connect(function()
        ...
        task.spawn(function()
            ...
            -- 1. Angkat semua pet aktif di kebun
            local inGarden = GetEquippedPetsInGarden()
            for _, gPet in ipairs(inGarden) do
            ...
            -- 2. Bersihkan juga sisa dari database resmi game DataService
            if DataService then
                local ok, data = pcall(function() return DataService:GetData() end)
                if ok and data and data.PetsData and data.PetsData.EquippedPets then
                    for _, u in ipairs(data.PetsData.EquippedPets) do
            ...
            -- Refresh seketika agar status [ACTIVE] hilang
            if refreshPetSelectionUI then
                task.defer(refreshPetSelectionUI)
            end
        end)
